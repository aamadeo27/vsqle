...
import { Col, Button, Modal, FormControl, FormGroup, ControlLabel, Form, ButtonGroup } from 'react-bootstrap';
...
export default class CreateProcedure extends React.Component {
	...
	save(){
		...
		const partitionedSQL = ` partition on table ${procedure.partitionTable} column ${procedure.partitionColumn}`;
		let ddl = null;
		if ( procedure.type === 'sql' ){
			ddl = `create procedure ${procedure.name} ${procedure.partitioned === 'true' ? partitionedSQL :''} as ${procedure.sql}`;
		} else {
			ddl = `create procedure ${procedure.partitioned === 'true' ? partitionedSQL :''} from class ${procedure.class}`;
		}
	...
	render(){
		...
		return <Modal show={this.props.show} onHide={() => {}} bsSize="large">
			...
			<Modal.Footer>
				{ this.state.error ? <span className="text-danger pull-left">{this.state.error.toString()}</span> : ''}
				<ButtonGroup>
