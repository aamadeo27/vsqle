...
import { Table, Button, Checkbox, Modal, FormControl, FormGroup, ControlLabel, Glyphicon, Form, ButtonGroup } from 'react-bootstrap'
...
export default class CreateTableDialog extends React.Component {
  ...
  save(){
    ...
    let sql = 'Create ' + type + ' ' + object.name;
    if ( object.stream ) {
      if ( ptk ) sql += ' partition on column ' + ptk;
      if ( object.exportTo.length > 0 ) sql += ' export to target ' + object.exportTo;
    }
    ...
    } else sql = sql.substring(0, sql.length - 1);

    sql += ');'

    if ( ptk && !object.stream ) sql += ` partition table ${object.name} on column ${ptk};`

    console.log("Save object", { object }, sql);
    this.props.close();
  }
    ...
    const types = ['VARCHAR','INTEGER','BIGINT','FLOAT','DOUBLE','TIMESTAMP','VARBINARY'].map( t => <option value={t} key={t}>{t}</option>);
